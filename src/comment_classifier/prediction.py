...
if __name__ == '__main__':
    ...
    if config.mode == 'train':
        ...
        train_loader = get_train_loaders(config.train_address, config.pretrained_model, config.batch_size)

        for e in range(config.epochs):
            train_loss, train_f1, train_pre, train_rec, train_each_f1, train_each_pre, train_each_rec \
                = train_model(model, loss_function, train_loader, optimizer, e)

            print('epoch: {}, train_loss: {}, train_f1: {}, train_pre: {}, train_rec: {}'.format(e + 1,
                                                                                                 train_loss, train_f1,
                                                                                                 train_pre, train_rec))
        ...
        # if os.path.exists(f'./dataset/prediction_dataset/{config.test_dataset}.{config.test_mode}'):
        #     os.remove(f'./dataset/prediction_dataset/{config.test_dataset}.{config.test_mode}')
        with open(f'./../seq2seq/dataset/{config.test_dataset}/{config.test_mode}/{config.test_dataset}.{config.test_mode}', 'w') as w:
            for json_line, label in tqdm(zip(json_data, predict_labels)):
                ...
                label_num[label] += 1
        # with open(f'./../seq2seq/dataset/{config.test_dataset}/{config.test_mode}/raw_code.{config.test_mode}', 'w') as a1:
        #     with open(f'./../seq2seq/dataset/{config.test_dataset}/{config.test_mode}/comment.{config.test_mode}', 'w') as a2:
        #         with open(f'./../seq2seq/dataset/{config.test_dataset}/{config.test_mode}/label.{config.test_mode}', 'w') as a3:
        #             for json_line, label in zip(json_data, predict_labels):
        #                 data_dict = json.loads(json_line.strip())
        #                 a1.write(data_dict['raw_code'].strip() + '\n')
        #                 a2.write(data_dict['comment'].strip() + '\n')
        #                 a3.write(config.class_name[label] + '\n')
        #
        #                 label_num[label] += 1
